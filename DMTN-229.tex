...
Registry tracks datasets and relationships but has no idea where or how datasets are persisted.
...
A dataset type uses a name to refer to the list of relevant dimensions, the ``storage class'' associated with the data itself, and whether or not it is a calibration dataset.
...
The ``storage class'' is how the Butler is informed about the underlying Python type associated with the dataset.
...
We do not make use of the Object Relational Mapping interface, and instead use the low-level SQLAlchemy interface to create tables.

The Registry stores all the dimension information, collection definitions, and knowledge of all the datasets.
...
On put datastore is given a reference to the dataset (encapsulated in the Python class \texttt{DatasetRef}) by Registry (this is the data coordinate, the dataset type, the run collection, and the unique identifier, a UUID, assigned by Registry) and the Python object.
...
To solve this problem we are working on a client/server Butler\cite{DMTN-176}.
...
Examples of the current subcommands and options are show in Fig.~\ref{fig:butler-cli}.
...
To enable that it must be converted into a \texttt{PipelineTask}.

A \texttt{PipelineTask} is special because it requires that the author declares the required input datasets and output datasets in a way that can be understood by a pipeline graph builder that is attached to a Butler registry.
Critically, a \texttt{PipelineTask} must declare the dimensions that it is interested in.
...
To support pipeline execution the \texttt{run()} method must now takes parameters that match the connections and must return a \texttt{dict}-like data structure where the keys match the expected outputs.
...
Each task can have specific configurations given as well as allowing the pipeline executor to take external user overrides.
...
Fig.~\ref{fig:graph}a shows a visualization of the demonstration pipeline shown in Fig.~{fig:pipeline}.
...
The user provides an input Butler collection to search for datasets, and a data query to select specific files from that collection.
